...
class PMW3901 {
    ...
    public:
                ...
                int16_t & deltaX, int16_t &  deltaY, bool & gotMotion) 
        {
            ...
            spi_exchange(1, &address, &address);
            delayMicroseconds(50);
            spi_exchange(sizeof(motionBurst_t), 
                    (uint8_t*)&_currentMotion, (uint8_t*)&_currentMotion);
            delayMicroseconds(50);
            digitalWrite(csPin, HIGH);
            spi_end_transaction();
            delayMicroseconds(50);
    ...
    protected:
        ...
        void spi_exchange(size_t size, uint8_t * data_tx, uint8_t * data_rx);
    ...
    private:
        ...
        void registerWrite(const uint8_t csPin, uint8_t reg, uint8_t value)
        {
            ...
            spi_exchange(1, &reg, &reg);
            delayMicroseconds(50);
            spi_exchange(1, &value, &value);
        ...
        uint8_t registerRead(const uint8_t csPin, uint8_t reg)
        {
            uint8_t data = 0;
            uint8_t dummy = 0;
